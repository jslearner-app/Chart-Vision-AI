...
export default function App() {
  ...
  const handleSelectKey = useCallback(async () => {
    try {
      if (window.aistudio) {
        ...
        // Optimistically set to true. If the key is bad, the API call will fail
        // and we'll guide the user to select a new one.
        setHasApiKey(true);
      }
    } catch (e) {
  ...
  const handleAnalyzeClick = async () => {
    ...
    try {
      ...
      // This error often indicates an invalid or incorrectly configured API key.
      if (errorMessage.includes('API key not valid') || errorMessage.includes('Requested entity was not found')) {
        setError('Your API Key appears to be invalid. Please select a valid key and try again.');
        setHasApiKey(false); // Re-prompt for key
      } else {
  ...
  if (!hasApiKey) {
    return <ApiKeyPrompt onSelectApiKey={handleSelectKey} />;
  }
